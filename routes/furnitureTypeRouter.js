...
.get((req, res, next) => {
    ...
    .then(furnitureType => {
        if (furnitureType){
            ...
            const err = new Error(`FurnitureType ${req.params.furnitureTypeId} not found`);
            err.status = 404;
            return next(err);
        }
    })
...
.delete((req, res, next) => {
    ...
    .then(furnitureType => {
        if(furnitureType){
            for (let i = furnitureType.furnitures.length - 1; i >= 0; i--){
                furnitureType.furnitures.id(furnitureType.furnitures[i]._id).remove();
            }
            furnitureType.save()
            .then(furnitureType => {
                res.statusCode = 200;
...
.get((req, res, next) => {
    ...
    .then(furnitureType => {
        if (furnitureType && furnitureType.furnitures.id(req.params.furnitureId)){
            res.statusCode = 200;
            res.setHeader("Content-Type", "application/json");
            res.json(furnitureType.furnitures.id(req.params.furnitureId));
        } else if (!furnitureType) {
            const err = new Error(`FurnitureType ${req.params.furnitureTypeId} not found`);
            err.status = 404;
            return next(err);
        } else {
            const err = new Error(`Furniture ${req.params.furnitureId} not found`);
            err.status = 404;
            return next(err);
        }
    })
...
.put((req, res, next) => {
    ...
    .then(furnitureType => {
        if(furnitureType && furnitureType.furnitures.id(req.params.furnitureId)){
            if (req.body.name){
                furnitureType.furnitures.id(req.params.furnitureId).name = req.body.name;
            }
            if (req.body.price){
                furnitureType.furnitures.id(req.params.furnitureId).price = req.body.price;
            }
            if (req.body.image){
                furnitureType.furnitures.id(req.params.furnitureId).image = req.body.image;
            }
            if (req.body.description){
                furnitureType.furnitures.id(req.params.furnitureId).description = req.body.description;
            }
            if (req.body.quantity){
                furnitureType.furnitures.id(req.params.furnitureId).quantity = req.body.quantity;
            }
            furnitureType.save()
            .then(furnitureType => {
                res.statusCode = 200;
...
.delete((req, res, next) => {
    ...
    .then(furnitureType => {
        if(furnitureType && furnitureType.furnitures.id(req.params.furnitureId)){
            furnitureType.furnitures.id(req.params.furnitureId).remove();
            furnitureType.save()
            .then(furnitureType => {
                res.statusCode = 200;
                res.setHeader("Content-Type", "application/json");
